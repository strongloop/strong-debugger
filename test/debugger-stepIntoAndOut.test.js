...
var SCRIPT_UNDER_TEST = l.fixture(function breakBeforeFunctionCall() {
  ...
  setInterval(function() {
    ...
    callme();
    console.log('back in main'); // line index 5

    function callme() {
      console.log('called'); // line index 8
...
l.runUsing(l.debugScript(SCRIPT_UNDER_TEST), function(client) {
  return client.verifyScenario(function(s) {
    ...
    s.sendRequest({ method: 'Debugger.stepInto' });
    s.expectResponse();
    s.expectEvent('Debugger.resumed');
    s.expectDebuggerPausedAt(SCRIPT_UNDER_TEST, 8);

    s.sendRequest({ method: 'Debugger.stepOut' });
    s.expectResponse();
    s.expectEvent('Debugger.resumed');
